...
$db->prepare("SetReplaysLatestHotsApiId",
    "UPDATE `pipeline_variables` SET `val_int` = ? WHERE `key_name` = \"replays_latest_hotsapi_id_secondary\"");
...
while (true) {
    ...
    if ($pipeconfigresrows > 0) {
        ...
        $r_key_name = "replays_latest_hotsapi_id_secondary";
        ...
        if ($lastCatalogedReplayResultRows > 0) {
            ...
            $maxCatalogedReplay = $lastCatalogedRow['val_int_2'];
            $db->freeResult($lastCatalogedReplayResult);

            if ($lastCatalogedReplay < $maxCatalogedReplay) {
                ...
                echo timestamp() . "Secondary Find Process has reached its target replay group of $maxCatalogedReplay.               \r";
                $sleep->add(OUT_OF_REPLAYS_COUNT_DURATION);
            }
        }
        else {
